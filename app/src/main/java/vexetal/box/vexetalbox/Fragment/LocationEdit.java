...
public class LocationEdit extends Fragment
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        FirebaseDatabase.getInstance().getReference().child("Users").child(session.getusername()).child("Address")
                .child(pushid)
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if(dataSnapshot.exists()){
                            ...
                            String a[]=dataSnapshot.child("Coord").getValue().toString().split(",");
                            LatLng latLng=new LatLng(Double.parseDouble(a[0]),Double.parseDouble(a[1]));

                            mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(latLng, DEFAULT_ZOOM));

                            coord.setText(dataSnapshot.child("Coord").getValue().toString());
                                ...
                                if(dataSnapshot.exists()){
                                    ...
                                    try {
                                        ...
                                        if (Math.round(results[0] / 1000) <= Double.parseDouble(slab1)) {
                                            DatabaseReference ref = FirebaseDatabase.getInstance().getReference().child("Users").child(session.getusername()).child("Address").child(pushid);
